...
class Batch(DotDict):
    ...
    def from_list(sequences: List[Sequence]) -> "Batch":
        ...
        for name in other_attr_names:
            ...
            type_mag = [categorize_magnitudes(value) for value in values]

            other_attr[name] = pad_sequence(
                values, padding_value=0, max_len=padded_seq_len
            )
            other_attr[f"{name}_type"] = pad_sequence(              # Added : mag_type, as in the other github, for njdtpp
                type_mag, padding_value=0, max_len=padded_seq_len
            )
        
        return Batch(
            inter_times=inter_times, #.tolist(),
            arrival_times=arrival_times,#.tolist(),
            t_start=t_start,#.tolist(),
            t_end=t_end,#.tolist(),
            t_nll_start=t_nll_start,#.tolist(),
            mask=mask,#.tolist(),
            start_idx=start_idx,#.tolist(),
            end_idx=end_idx,#.tolist(),
            **other_attr,#**{k: v.tolist() for k, v in other_attr.items()}, #**other_attr,
        ) 
        '''
        return Batch(
                inter_times=inter_times.cpu().numpy().astype(np.float64),
                arrival_times=arrival_times.cpu().numpy().astype(np.float64),
                t_start=t_start.cpu().numpy().astype(np.float64),
                t_end=t_end.cpu().numpy().astype(np.float64),
                t_nll_start=t_nll_start.cpu().numpy().astype(np.float64),
                mask=mask.cpu().numpy().astype(np.float64),
                start_idx=start_idx.cpu().numpy().astype(np.float64),
                end_idx=end_idx.cpu().numpy().astype(np.float64),
                **{k: v.cpu().numpy().astype(np.float64) for k, v in other_attr.items()}, #**other_attr,
            )
        '''
...
#***** ADDED FUNCTION for mag_type and njdtpp **********************
def categorize_magnitudes(magnitudes):
    # Convertir en torch.Tensor si ce n'est pas déjà un Tensor
    ...
    # Convertir les magnitudes en catégories
    categories = torch.bucketize(magnitudes, torch.tensor(thresholds), right=False)
