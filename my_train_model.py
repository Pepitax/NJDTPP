...
def train_model(
    ...
    if model_name == "ETAS":
        ...
        if batch_size is None:
            batch_size = 1
        dl_train = catalog.train.get_dataloader(batch_size=batch_size, shuffle=True)   # for recast: WHY IS THE SHUFFLE TRUE HERE !?!?!?
        ...
        print(device_)
        model = NJDTPP(
            dim_eta=7, #from the datas, 7 types of event for multivariate hawkes process
            dim_hidden=32,
            num_hidden=2, 
            sigma=0.01,
            #activation=nn.Tanh()
    ...
    trainer = pl.Trainer(
        logger=logger,
        gpus= int(torch.cuda.is_available() and use_gpu), #0       #put 0 for cpu, njdtpp was faster on the other github with cpu
        max_epochs=max_epochs,
        callbacks=[checkpoint, early_stopping],
        accumulate_grad_batches=accumulate_grad_batches,
        precision=precision,
        enable_progress_bar=False, 
        #gradient_clip_val=10, # some instability in the loss calculations, instead i clipped the integral term in euler_solver in the njdtpp class
    )
    ...
    # Evaluate the model on CPU     #we tried on gpu but it is not much more fast so we let the cpu
    tester = pl.Trainer(
        gpus=0, #int(torch.cuda.is_available() and use_gpu), # #accelerator=accelerator_, #gpus=int(torch.cuda.is_available() and use_gpu),
        logger=False,
